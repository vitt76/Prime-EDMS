...
export const notificationService = {
  getNotifications(params?: {
    ...
    // NOTE: Optional debug telemetry (disabled by default).
    if (import.meta.env.VITE_DEBUG_INGEST === 'true') {
      fetch('http://127.0.0.1:7242/ingest/e2a91df7-36f3-4ec3-8d36-7745f17b1cac', {
        ...
        body: JSON.stringify({
          ...
          runId: 'run1',
          hypothesisId: 'D',
          location: 'services/notificationService.ts:getNotifications(request)',
      ...
      if (import.meta.env.VITE_DEBUG_INGEST === 'true') {
        fetch('http://127.0.0.1:7242/ingest/e2a91df7-36f3-4ec3-8d36-7745f17b1cac', {
          ...
          body: JSON.stringify({
            ...
            runId: 'run1',
            hypothesisId: 'D',
            location: 'services/notificationService.ts:getNotifications(response)',
      ...
      if (import.meta.env.VITE_DEBUG_INGEST === 'true') {
        fetch('http://127.0.0.1:7242/ingest/e2a91df7-36f3-4ec3-8d36-7745f17b1cac', {
          ...
          body: JSON.stringify({
            ...
            runId: 'run1',
            hypothesisId: 'D',
            location: 'services/notificationService.ts:getNotifications(error)',
            message: 'Frontend: API error in getNotifications',
            data: { error: String(error), error_message: (error as any)?.message, error_status: (error as any)?.response?.status },
            timestamp: Date.now()
          })
        }).catch(() => {})
      }
