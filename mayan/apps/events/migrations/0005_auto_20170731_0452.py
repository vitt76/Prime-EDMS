...
def code_update_event_types_names(apps, schema_editor):
...
def code_revert_event_types_names(apps, schema_editor):
    Action = apps.get_model(app_label='actstream', model_name='Action')
    StoredEventType = apps.get_model(
        app_label='events', model_name='StoredEventType'
    )
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            code=code_update_event_types_names,
            reverse_code=code_revert_event_types_names
        )
    ]
