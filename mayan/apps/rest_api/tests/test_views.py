...
class RESTAPIViewTestCase(RESTAPIViewTestMixin, GenericViewTestCase):
    ...
    def test_openapi_ui_view(self):
        response = self._request_test_openapi_ui_view()
        self.assertEqual(response.status_code, 200)

    def test_openapi_schema_view(self):
        # drf-spectacular returns an OpenAPI JSON document.
        self.expected_content_types = (
            'application/vnd.oai.openapi; charset=utf-8',
            'application/json; charset=utf-8',
        )

        response = self._request_test_openapi_schema_view()
        self.assertEqual(response.status_code, 200)
