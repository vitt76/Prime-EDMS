...
### Фактическое состояние (08.12.2025)

- SPA работает напрямую через Vite‑прокси на `/api/v4/*` против работающего Mayan; `authService` (token), `assetService` и upload поток (simple + chunked) успешны.
- Админ‑панель переведена на реальные REST Mayan: Users/Groups/Schemas/Workflows запрашиваются с пагинацией и поиском, виджеты Dashboard получают данные через `/api`.
- Получение текущего пользователя и лента документов (`/api/v4/users/current/`, `/api/v4/documents/optimized/`) возвращают корректный JSON (на стенде сейчас пустой список документов).
- Оставшиеся разрывы: смена пароля (нет endpoint), динамическая конфигурация метаданных/форм загрузки, персональная лента активности — всё ещё требует headless/BFF.
- Коллекции/галерея: страницы «Мои загрузки/Избранное/Недавние/Доступные мне» переведены на реальные API (headless my_uploads, favorites, accessed, documents ACL). Моки отключены.
...
| **Админ (Users/Groups/Schemas/Workflows)** | ✅ Работает | Прямые REST Mayan `/api/v4/...` | Добавлены реальные вызовы, пагинация/поиск |
...
| `src/services/adminService.ts` | `GET /api/v4/users|groups|workflow_templates|metadata/...` | Без изменений | ✅ Работает (подключено к реальному REST, пагинация/поиск) |
...
└── adminService.ts           ✅ РАБОТАЕТ (подключен к Mayan `/api/v4/...`, поиск/пагинация)
