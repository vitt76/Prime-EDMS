...
### Фактическое состояние окружения (08.12.2025)

- Контейнеры `app/postgresql/redis/rabbitmq` в состоянии **healthy**; `/api/v4/` отвечает JSON, Vite прокси успешно прокидывает запросы.
- Базовые REST работают: `GET /api/v4/users/current/` возвращает пользователя; `GET /api/v4/documents/optimized/` отдает пустой список, но 200 OK; upload (init + chunked + complete) завершается успешно и создает документы.
- Админ REST (`/api/v4/users/`, `/api/v4/groups/`, `/api/v4/workflow_templates/`, `/api/v4/metadata/metadata_types/`) возвращает данные и используется фронтендом для пагинации/поиска.
- Основные незакрытые дыры: нет JSON endpoint для смены/сброса пароля, нет headless-конфигурации типов/обязательных метаданных, нет пользовательской ленты активности; при 401/302 Stronghold возвращает HTML.
- Добавлен headless `/api/v4/headless/documents/my_uploads/` (по событиям actor), chunked upload теперь проставляет `_event_actor` для Document/DocumentFile, чтобы корректно строить выборку «Мои загрузки».
